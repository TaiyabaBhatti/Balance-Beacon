...
import { generateGrid, print } from "../../encrypt/methods";
import { gridPattern } from "../../encrypt/decryptmetadata";
...
export default function EditProfile() {
  ...
  const editProfile = async (data) => {
    ...
    try {
      ...
      let grid = gridPattern(10);
      let metaData = [];
      const cipheredName = executeAlgorithm(
        grid,
      ...
      metaData.push({
        cipher: cipheredName, // the encrypted data
        direction: "x", // horizontal or vertical
        row: 3,
        col: 6,
        length: data.name.replace(/\s/g, "").length, // length of original data
        gridSize: 10,
        preperty: "name", // for regenerating grid
      });
      console.log(metaData);
      const cipheredGender = executeAlgorithm(
        grid,
        data.gender,
        "y",
        5,
      ...
      const cipheredAge = executeAlgorithm(grid, data.age, "y", 7, 3, "age");
      ...
      <PatientProfile path="/dashboard">
        <div className="relative">
          <div className={loading ? "blur-sm" : ""}>
            <form
              className="flex flex-col text-black gap-y-2 w-fit md:w-[555px]"
              action="post"
            ...
            >
              ...
              <span className="text-white text-right  text-sm ">
                Forget Password? <Link className="font-bold">Send me code</Link>
              </span>
